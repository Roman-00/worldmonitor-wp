...
if ( ! function_exists( 'worldmonitor_setup' ) ) :
	...
	function worldmonitor_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'worldmonitor' ),
			)
		);
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'worldmonitor_custom_background_args',
				array(
 ...
 */
function worldmonitor_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'worldmonitor_content_width', 640 );
}
add_action( 'after_setup_theme', 'worldmonitor_content_width', 0 );
 ...
 */
function worldmonitor_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'worldmonitor' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'worldmonitor' ),
...
add_action( 'widgets_init', 'worldmonitor_widgets_init' );
